...
<head>
    ...
    <title>Contactar con Ofertante</title>
   ...
   <h2>Contactar</h2>
    ...
    <form id="contactarForm" class="control-form" action="/marketplace/app/controllers/actionsController.php" method="POST" enctype="multipart/form-data">
        <div class="row mb-3">
            ...
            <input type="text" class="form-control" id="titulo" name="titulo" readonly value="<?= htmlspecialchars($card['titulo']) ?>">
        </div>
        <div class="row mb-3">
        ...
        <?php           
            ...
            //id del usuario que recibirá el mensaje
            $id_receptor = $_SESSION['detalleByCard'][0]['id_usuario'];
        ?>
            <!-- enviamos al controlador los datos de emisor y del receptor, así como el mensaje -->
            <input type="hidden" name="action" id="action" value="contacUser">
            <input type="hidden" name="id_emisor" id="id_emisor" value="<?= htmlspecialchars($id_emisor) ?>">
            <input type="hidden" name="id_receptor" id="id_receptor" value="<?= htmlspecialchars($id_receptor) ?>">
            <input type="hidden" name="email_receptor" id="email_receptor" value="<?= $detalleByCard[0]['email'] ?>">
